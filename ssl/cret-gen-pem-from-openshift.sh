...
echo "oc delete csr ${CSR_NAME} || :"
oc delete csr ${CSR_NAME} || :
...
while true; do
  ...
  echo "oc get csr ${CSR_NAME}"
  oc get csr ${CSR_NAME} > /dev/null 2>&1
  if [ "$?" -eq 0 ]; then
      break
  fi
...
while true; do
  ...
  echo "oc get csr ${CSR_NAME} -o jsonpath='{.status.certificate}'"
